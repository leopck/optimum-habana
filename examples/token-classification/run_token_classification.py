...
import evaluate
import numpy as np
from habana_frameworks.torch.hpu import graphs
from custom_graphs import CustomGraphModel
...
graphs.GraphModel = CustomGraphModel
...
def main() -> None:
    ...
    # Check the model's forward inputs against the dataset keys
    model = LiltForTokenClassification.from_pretrained(
        model_id, num_labels=len(labels), label2id=label2id, id2label=id2label
    )

    expected_inputs = {'input_ids', 'attention_mask', 'bbox', 'labels'}
    dataset_keys = set(proc_dataset["train"].features.keys())

    missing_inputs = expected_inputs - dataset_keys
    if missing_inputs:
        logger.error(f"Missing inputs in the dataset: {missing_inputs}")
        raise ValueError(f"Dataset is missing the following required inputs: {missing_inputs}")

    extra_inputs = dataset_keys - expected_inputs
    if extra_inputs:
        logger.warning(f"Extra inputs in the dataset that are not used by the model: {extra_inputs}")
